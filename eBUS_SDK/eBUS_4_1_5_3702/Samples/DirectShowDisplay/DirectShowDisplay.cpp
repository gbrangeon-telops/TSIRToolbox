...
int main()
{
    ...
    lWnd.ShowModal();
    const PvDeviceInfo *lDeviceInfo = lWnd.GetSelected();
    if ( lDeviceInfo == NULL )
    {
        return -1;
    }
    ...
    //
    std::cout << "Configuring and connecting eBUS SDK PvDSSource filter to " << lDeviceInfo->GetDisplayID().GetAscii() << std::endl;
    BSTR lDeviceID = ::SysAllocStringLen( lDeviceInfo->GetConnectionID().GetUnicode(), lDeviceInfo->GetConnectionID().GetLength() );
