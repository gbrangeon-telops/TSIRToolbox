             ...
             ( mSetup->GetRole() == Setup::RoleData ) )
        {
            ...
            if ( mSetup->GetDestination() == Setup::DestinationUnicastAuto )
            {
                lResult = mStream->Open( 
                    mDeviceInfo->GetIPAddress(), 0, 0, lNetworkAdapter->GetIPAddress() );
            }
            else if ( mSetup->GetDestination() == Setup::DestinationUnicastSpecific )
            {
                lResult = mStream->Open(
                    mDeviceInfo->GetIPAddress(), mSetup->GetUnicastSpecificPort(), 0, lNetworkAdapter->GetIPAddress() );
            }
            else if ( mSetup->GetDestination() == Setup::DestinationMulticast )
            {
                lResult = mStream->Open(
                    ...
                    mSetup->GetMulticastPort(), 0, lNetworkAdapter->GetIPAddress() );
            }
            else
            {
                ASSERT( 0 );
