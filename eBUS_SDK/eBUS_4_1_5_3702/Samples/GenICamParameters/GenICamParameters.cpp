...
//
// Have the user select a device, return a connection ID string
//

bool Select( PvString &aConnectionID )
{
    PvResult lResult;
    ...
    aConnectionID = lDeviceInfo->GetConnectionID();
...
PvDevice *Connect( const PvString &aConnectionID )
{
    ...
    cout << "Connecting to " << aConnectionID.GetAscii() << endl;
    PvDevice *lDevice = PvDevice::CreateAndConnect( aConnectionID, &lResult );
    if ( !lResult.IsOK() )
    {
        cout << "Unable to connect to " << aConnectionID.GetAscii() << endl;
...
bool GetDeviceSettings( const PvString &aConnectionID )
{
...
bool GetImageStreamControllerSettings( PvString &aConnectionID )
{
    PvString lDeviceID;
    PvResult lResult = PvResult::Code::INVALID_PARAMETER;

    // Creates stream object
    cout << "Opening stream from " << aConnectionID.GetAscii() << endl;
    PvStream* lStream = PvStream::CreateAndOpen( aConnectionID, &lResult );
    if ( ( lStream == NULL ) || !lResult.IsOK() )
    {
        cout << "Error creating and opening stream from " << aConnectionID.GetAscii() << endl;
