...
BEGIN_MESSAGE_MAP(CTransmitTiledImagesDlg, CDialog)
    ...
    ON_WM_SIZE()
    ON_WM_GETMINMAXINFO()
    ON_STN_CLICKED(IDC_STATIC_MAC_22, &CTransmitTiledImagesDlg::OnStnClickedStaticMac22)
END_MESSAGE_MAP()
...
BOOL CTransmitTiledImagesDlg::OnInitDialog()
{
    ...
    for ( uint32_t i = 0; i < lSystem.GetInterfaceCount(); i++ )
    {
        ...
        if ( lNetworkAdapter != NULL )
        {
            ...
            {
                mComboIfAddress.AddString( lNetworkAdapter->GetIPAddress().GetUnicode() );
                lFound = true;
            }
        }
    }
...
void CTransmitTiledImagesDlg::OnStnClickedStaticMac22()
{
