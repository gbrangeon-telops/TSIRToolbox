...
========================
Using PvTransmitTiledImageSample
========================

This sample illustrates how to use the eBUS SDK to receive, transform, and transmit images. More specifically, it is an MFC GUI application that receives video from up to four different GigE Vision transmitters, tiles the images together, transmits the output to a given destination, and finally displays a the tiled output to the monitor. This sample is designed more for use as an application than as illustrative source code. As a result, it is recommended that application developers begin by reading and understanding a simpler example such as PvTransmitTestPatternSample before attempting to dissect the source code of PvTransmitTiledImageSample.

1. Introduction

Basic instructions for using the sample:
	1. Start PvTransmitTiledImage.
	2. Click one of the four button "Connect/Open" to select a GigE Vision transmitter.
	3. Select a GigE Vision transmitter from the list of available devices (should be on the same subnet as the network interface you wish to receive on).
	4. You can use the buttons "..." to customized the configuration of any GigE Vision transmitter.
	5. Repeat the stepS 2 and 3 to connect up to 4 GigE Vision devices.
 ...
 * You have a GigE Vision receiver that can receive and display the transmitted data(such as a vDisplay, GEVPlayer, NetCommand or any other GigE Vision receiver that supports the GVSP protocol). The receiver should be reachable and on the same subnet as the interface from which it will be receiving.
 * You have at least one GigE Vision transmitter connected to your PC. The transmitter should be reachable and on the same subnet as the interface to which it will be transmitting.
...
Each of the GigE Vision transmitter source are managed by independant threads ( StreamThread ). The latest image acquire by the thread is stored in the SmartBuffer who is stored in the CurrentBuffersTable. 
