...
void ControlPane::SetDocument( NetCommandDoc *aDoc )
{
    if ( mDocument != NULL )
    {
        PvGenEnum *lMode = dynamic_cast<PvGenEnum *>( mDocument->GetDevice()->GetGenParameters()->Get( "AcquisitionMode" ) );
        if ( lMode != NULL )
        {
...
void ControlPane::EnableInterface()
{
    ...
    {
        lMode = mDocument->GetDevice()->GetGenParameters()->Get( "AcquisitionMode" );
        lStart = mDocument->GetDevice()->GetGenParameters()->Get( "AcquisitionStart" );
        lStop = mDocument->GetDevice()->GetGenParameters()->Get( "AcquisitionStop" );
    }
...
void ControlPane::OnCbnSelchangeMode()
{
    ...
    PvUInt64 lValue = mObjectCombo.GetItemData( mObjectCombo.GetCurSel() );
