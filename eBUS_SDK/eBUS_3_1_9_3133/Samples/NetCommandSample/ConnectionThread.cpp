...
// ==========================================================================
ConnectionThread::ConnectionThread( Setup *aSetup, PvDeviceInfo *aDeviceInfo, PvDevice *aDevice, PvStream *aStream, CWnd* aParent )
        : mSetup( aSetup )
...
DWORD ConnectionThread::Function()
{
    ...
    try
    {
             ...
             ( mSetup->GetRole() == Setup::RoleCtrl ) )
        {
            ...
            if ( mSetup->GetRole() == Setup::RoleCtrlData )
            {
                ...
                else
                {
					...
					PvGenInteger *lPacketSize = dynamic_cast<PvGenInteger *>( mDevice->GetGenParameters()->Get( "GevSCPSPacketSize" ) );
                    if ( lPacketSize != NULL )
                    {
             ...
             ( mSetup->GetRole() == Setup::RoleData ) )
        {
            ...
            {
                lResult = mStream->Open(
                    mDeviceInfo->GetIPAddress(), mSetup->GetUnicastSpecificPort(), 0, mDeviceInfo->GetInterface()->GetIPAddress() );
            }
            else if ( mSetup->GetDestination() == Setup::DestinationMulticast )
            {
                lResult = mStream->Open(
                    ...
                    mSetup->GetMulticastPort(), 0, mDeviceInfo->GetInterface()->GetIPAddress() );
            }
            else
            {
                ASSERT( 0 );
