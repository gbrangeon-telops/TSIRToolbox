...
void CPropertiesWnd::ConfigureBrowser()
{
    ...
    else
    {
        ...
        case DATA_COMMLINKPARAMS:
            if ( mCurrentArray != mDocument->GetDevice()->GetGenLink() )
            {
                mCurrentArray = mDocument->GetDevice()->GetGenLink();
                mGenBrowserWnd.SetGenParameterArray( mCurrentArray );
            }
            break;

        case DATA_DEVICEPARAMS:
            if ( mCurrentArray != mDocument->GetDevice()->GetGenParameters() )
            {
                mCurrentArray = mDocument->GetDevice()->GetGenParameters();
                mGenBrowserWnd.SetGenParameterArray( mCurrentArray );
            }
            break;
