// ActionCommandWnd.cpp : implementation file
//
...
void CHexEdit::SetValue( PvUInt32 aValue, bool aHex )
{
    CString lTemp;
    if( aHex )
    {
...
void CHexEdit::SetValue( PvUInt64 aValue, bool aHex )
{
    CString lTemp;
    if( aHex )
    {
...
PvUInt32 CHexEdit::GetValueUInt32()
{
    PvUInt32 lValue = 0;
    CString lText;
    bool lHex = false;
...
PvUInt64 CHexEdit::GetValueUInt64()
{
    PvUInt64 lValue = 0;
    CString lText;
    bool lHex = false;
...
// ActionCommandWnd dialog

IMPLEMENT_DYNAMIC(ActionCommandWnd, CDialog)

ActionCommandWnd::ActionCommandWnd(CWnd* pParent /*=NULL*/)
	: CDialog(ActionCommandWnd::IDD, pParent)
{

}

ActionCommandWnd::~ActionCommandWnd()
{
}

void ActionCommandWnd::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(ActionCommandWnd, CDialog)
    ON_BN_CLICKED(IDC_CHECKSCHEDULED, &ActionCommandWnd::OnBnClickedCheckscheduled)
    ON_BN_CLICKED(IDC_BUTTONSEND, &ActionCommandWnd::OnBnClickedButtonsend)
END_MESSAGE_MAP()


// ActionCommandWnd message handlers

BOOL ActionCommandWnd::OnInitDialog()
{
    ...
    for( PvUInt32 i = 0; i < mActionCommand.GetInterfaceCount(); i++ )
    {
    ...
    PvUInt64 lScheduledTime = mActionCommand.GetScheduledTime();
...
void ActionCommandWnd::OnBnClickedCheckscheduled()
{
...
void ActionCommandWnd::OnBnClickedButtonsend()
{
    ...
    if( lResult.IsOK() )
    {
        ...
        for( PvUInt32 i = 0; i < mActionCommand.GetAcknowledgeCount(); i++ )
        {
            mActionCommand.GetAcknowledgeStatus( i, lStatus );
            mActionCommand.GetAcknowledgeIPAddress( i, lIPAddress );
...
void ActionCommandWnd::OnOK()
{
    // Do nothing, just ensure not calling the parent
}

void ActionCommandWnd::OnCancel()
{
    DestroyWindow();
}
