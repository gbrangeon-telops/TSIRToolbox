...
// PvTransmitterRaw classes to transmit the contents of the screen using the 
// eBUS SDK.
// 
...
// 
// By default, this sample transmits the contents of the screen from the first 
// valid interface to multicast address 239.192.1.1:1042. However, it can be 
// used in many different ways by providing it with optional command line 
// arguments. For further details on the command line options that are 
// available, run the sample with the argument --help.
// *****************************************************************************
    ...
    while( !PvKbHit() )
    {
        ...
        if ( ( lConfig.GetFPS() == 0 ) || lStabilizer.IsTimeToDisplay( (PvUInt32)lConfig.GetFPS() ) )
        {
            ...
            if ( lFreeBuffers.size() > 0 )
            {
                ...
                // Grab screen into PvBuffer
                lSource.CaptureScreen( lBuffer );
